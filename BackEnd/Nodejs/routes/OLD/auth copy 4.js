  ...
  try {
    ...
    jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
        ...
        db.query(queryGetNewUserData, [email, token], (err, result) => {
            ...
            else if(UserType == "ServiceProvider") res.redirect(`http://localhost:5173/NewServiceProviderForm${queryString}`); 
...
const uploadsStartupDirectory = path.join(__dirname, "../../Storage/StartupStorage/PitchDeck");
if (!fs.existsSync(uploadsStartupDirectory)) {
  fs.mkdirSync(uploadsStartupDirectory, { recursive: true });
}

// Set up multer storage
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, uploadsStartupDirectory);
  },
...
const uploadStartup = multer({ storage });

// Route to handle file and form data submission
router.post("/createStartupProfile", uploadStartup.single("file"), async (req, res) => {
...
// Ensure the uploads directory exists, and if not, create it
const uploadsServiceDirectory = path.join(__dirname, "../../Storage/ServiceProviderStorage/Logo");
if (!fs.existsSync(uploadsServiceDirectory)) {
  fs.mkdirSync(uploadsServiceDirectory, { recursive: true });
}

// Set up multer storage
const storageService = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, uploadsServiceDirectory);
  },
  filename: (req, file, cb) => {
    cb(null, Date.now() + path.extname(file.originalname)); // Unique filename with extension
  },
...
const uploadServiceLogo = multer({ storageService });



// Route to handle file and form data submission
router.post("/createServiceProviderProfile", uploadServiceLogo.single("file"), async (req, res) => {
  // console.log(req.body.token);

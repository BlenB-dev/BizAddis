...
const StartupEditForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
       const response = await axios.post("http://localhost:5000/api/auth/editStartupProfile", formPayload, {
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      axios.get('http://localhost:5000/api/GetStartupProfile' ,{
          headers: {
            ...
            const FilePath = response.data[0].PickDeckURL;
            ...
            if(FilePath != undefined && FilePath.length > 5) {
              ...
              axios.post('/api/getProfilePic',formPayload, { 
                headers: {
  ...
  {ErrorMessage || ResponseMessage ? (
    <div>
      ...
      <Box display="flex" flexDirection="column" justifyContent="center" alignItems="center">
        ...
        <Button
          variant="outlined"
          onClick={(e) =>{e.preventDefault(); setLoading(false); setResponseMessage(null); 
          }}
          style={{ marginTop: '10px' }}
        >
          Close

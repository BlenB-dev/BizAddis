...
const StartupDetails = () => {
        ...
        headers: {
          ...
          "Content-Type": "application/json",
            ...
            if(response.data.length>0){
                ...
                await axios.post("http://localhost:5000/api/GetConnectionStatusForInvestorByStartupID", newForm, {
                  headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                  },
                      ...
                      //console.log(response.data[0]);
                      if(response.data.length>0){
                          console.log("Number of Retrived Connect Status:  "+response.data.length)
                          console.log(response.data);
                          // setUserData(response.data[0]);
                          setConnectType(response.data[0].ConnectType)
                          setConnectReply(response.data[0].ConnectReply)
                          console.log("Connect Type: "+response.data[0].ConnectType + "\nConnect Reply: "+response.data[0].ConnectReply)
                          console.log("Connect Type: "+ConnectType + "\nConnect Reply: "+ConnectReply)
          
                          
                  }else console.log("Retrived No Connect Status: "+response.data.length)  
                })
                .catch((err) => {
                  console.log(err);                  
                })
  ...
  const  ConnectToStartup =  () => {
    console.log("Connecting To Startup");
    ...
    axios.post("http://localhost:5000/api/InvestorConnectToStartup", newForm, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
  ...
  const  IgnoreStartup =  () => {
    ...
    axios.post("http://localhost:5000/api/InvestorIgnoreStartup", newForm, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
    ...
    <div className="mx-auto mt-[-70px] p-12 rounded-lg sm:w-full sm:ml-0 sm:h-auto">
        <div className="mb-12">
          <div className="flex flex-col items-start mb-6">
            ...
            {(ConnectType == null || ConnectType.length == 0) ? (
              <>
                  <div >
                    <button 
                      className="bg-white-500 text-red-500 px-4 py-2 mx-5 rounded border border-red-500 hover:bg-red-500 hover:text-white"
                      onClick={(e)=>{e.preventDefault(); IgnoreStartup();}}
                      >
                      Ignore
                      </button>
                    <button 
                      className="bg-white-500 text-blue-500 px-4 py-2 mx-5 rounded border border-blue-500 hover:bg-blue-500 hover:text-white"
                      onClick={(e)=>{e.preventDefault(); ConnectToStartup();}}
                      >
                      Connect</button>
                  </div>
              ...
              <>
              {ConnectReply === 'Unanswered' ? (
                <div >
                    <p className="bg-white-500 text-green-500 px-4 py-2 mx-5 rounded border border-green-500">Startup Has Not Answered Connection</p>
                  </div>

                ) : ConnectReply === 'Accept' ? (
                  <div >
                    <p className="bg-white-500 text-blue-500 px-4 py-2 mx-5 rounded border border-blue-500">Startup Has Accepted Connection</p>
                  </div>

                  
                ) : ConnectReply === 'Decline' ? (
                  <div >
                    <p className="bg-white-500 text-red-500 px-4 py-2 mx-5 rounded border border-red-500">Startup Has Declined Connection</p>
                  </div>
              ...
              <>
                <div >
                    <p className="bg-white-500 text-black-500 px-4 py-2 mx-5 rounded border border-grey-500">This Startup is Ignored</p>
                  </div>
              </>
            ):(
              <></>

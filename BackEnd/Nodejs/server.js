...
const GetServiceProviderRecommendationsForStartups = async (StartupUserID) => {
  try {
    ...
    const ServiceProviders = allServiceProviders.map(
      (item) => item.ServiceProviderID
    );
    ...
    if (startupVisitHistoryResult.length === 0) {
      ...
      for (
        let i = 0;
        i < ServiceProviders.length && Recommended.length < 25;
        i++
      ) {
        if (!Recommended.includes(ServiceProviders[i]))
          Recommended.push(ServiceProviders[i]);
      }
      return Recommended;
    }
    startupVisitHistory = startupVisitHistoryResult.map(
      (item) => item.ServiceProviderID
    );
    ...
    similarSearchStartups = similarSearchStartupsResults.map(
      (item) => item.StartupID
    );
    ...
    if (similarSearchServiceProviderResults.length === 0) {
      ...
      for (
        ...
        if (
          !Recommended.includes(ServiceProviders[i]) &&
          !startupVisitHistory.includes(ServiceProviders[i])
        )
          Recommended.push(ServiceProviders[i]);
      }
      return Recommended;
    }
    similarSearchServiceProvider = similarSearchServiceProviderResults.map(
      (item) => item.ServiceProviderID
    );
    for (
      let i = 0;
      i < similarSearchServiceProvider.length && Recommended.length < 25;
      i++
    ) {
      if (
        !Recommended.includes(similarSearchServiceProvider[i]) &&
        !startupVisitHistory.includes(ServiceProviders[i])
      )
        Recommended.push(similarSearchServiceProvider[i]);
    }
  ...
  try {
    ...
    const similarities = UnConnectedStartups.map((startup) => {
      const startupFeatures = [
        parseInt(startup.StageOfDevelopment),
        parseInt(startup.IndustrySector),
        parseInt(startup.FundingRequirement),
        parseInt(startup.CurrentFundingStatus),
        parseInt(startup.BusinessModel),
        parseInt(startup.RevenueModel),
        parseInt(startup.SubCity),
        parseInt(startup.CompetitiveAdvantage),
        parseInt(startup.SocialImactGoals),
      ];
      const similarityScore = cosineSimilarity(
        startupFeatures,
        JSON.parse(InvestorPreferences)
      );
    ...
    if (TotalConnected === 0 || IgnoreCount === 0) {
      ...
      const top25StartupIDs = top25SimilarStartups.map(
...
`;
      ...
      const listConnected = [
        connectedStartups.length,
        accpetedStartups.length,
        declinedStartups.length,
        ignoredStartups.length,
      ];

      const LeastConnected = Math.min(
        ...listConnected.filter((num) => num !== 0)
      );
      ...
      const InterActedStartups = combinedArrays
        .flat()
        .map((item) => [
      ...
      const InterActedStartupsSource = startupsWithSource
        .flat()
        .map((item) => parseInt(item.source));
      ...
      const ClassifedStartups = response.data
        .flat()
        .map((item) => [item.id, item.predicted_cluster]);
      ClassifedStartups.sort((a, b) => a[1] - b[1]);
      //console.log(ClassifedStartups);

      let Recommendation = [];
      for (let i = 0; i < ClassifedStartups.length; i++) {
        if (ClassifedStartups[i][1] != 3)
          Recommendation.push(ClassifedStartups[i][0]);
      ...
      if (Recommendation.length < 25) {
        for (
          let i = 0;
          Recommendation.length < 25 && i < similarityScoredIDs.length;
          i++
        ) {
          if (!Recommendation.includes(similarityScoredIDs[i]))
            Recommendation.push(similarityScoredIDs[i]);
        }
      }
...
async function saveInvestorToDatabase(data) {
  ...
  data.forEach((investor) => {
    ...
    const queryRegisterUsers =
      "INSERT INTO Users (username, email, password, userType, is_active) VALUES (?, ?, ?, ?, ?)";
    db.query(
      queryRegisterUsers,
      [
        investor.UserName,
      ...
      (err, result) => {
        ...
        db.query(getNewUserID, [investor.UserEmail], (err, result) => {
          if (err) {
            console.log(
              "Database Error Create INVESTOR Profile 4\n" + JSON.stringify(err)
            );
          }
    ...
    if (data.length > 0) {
      ...
      db.query(query2, [userID, ServiceProviderID], (err, data) => {
        if (err) console.log("Error Logging Vist" + err);
          ...
          console.log(
            "UserID: " + userID + " visited ServiceID: " + ServiceProviderID
          );
      });
...
app.post("/api/GetInvestorProfileByID", authenticateJWT, (req, res) => {
  ...
  db.query(sql, [InvestorID], (err, data) => {
    ...
    if (err) console.log("Unkown ERROr 2" + err);
    if (data.length > 0) {
      console.log("UserID: " + userID + " visited InvestorID: " + InvestorID);
    } else console.log("Retrived Data Length : " + data.length);
    return res.json(data);
  });
});
    ...
    if (data.length > 0) {
      ...
      db.query(query2, [userID, StartupID], (err, data) => {
        if (err) console.log("Error Logging Vist" + err);
          ...
          console.log("UserID: " + userID + " visited StartupID: " + StartupID);
      });
    } else console.log("Retrived Data Length : " + data.length);
    return res.json(data);
  });
...
app.post("/api/GetFilteredStartupProfiles", authenticateJWT, (req, res) => {
  ...
  let ParseInSoD =
    InSoD === undefined || InSoD === null ? " " : JSON.parse(InSoD);
...
app.get("/api/GetRecommendedStartups", authenticateJWT, async (req, res) => {
  ...
  const UserType = req.user.userType;
  try {
    const RecommendedStartups = await GetStartupRecommendationForInvestors(
      userID
    );
    console.log(
      "Amount of Recommended Startups: " + RecommendedStartups.length
    );
...
app.get(
  "/api/GetRecommendedServiceProviders",
  authenticateJWT,
  async (req, res) => {
    ...
    try {
      const RecommendedServiceProviders =
        await GetServiceProviderRecommendationsForStartups(userID);
      console.log(
        "Amount of Recommended Service: " + RecommendedServiceProviders.length
      );
...
app.post(
  "/api/GetConnectionStatusForInvestorByStartupID",
  authenticateJWT,
  (req, res) => {
    ...
    if (UserType != "Investor") {
      return res.status(403).json("Invalid User Type");
    }
    const query = `
    SELECT isc.*
    FROM InvestorStartupConnection isc
...
`;
  ...
  db.query(
    query,
    [StartupID, connectType, connectReply, userID, StartupID],
    (error, results) => {
      if (error) {
        ...
        return res
          .status(200)
          .json({ message: "Record already exists. No insert made" });
      }
    }
  );
});
...
app.post(
  "/api/GetConnectionStatusForStartupByInvestorID",
  authenticateJWT,
  (req, res) => {
...
app.post(
  "/api/StartupDeclineInvestorConnection",
  authenticateJWT,
  (req, res) => {
...
`;
    ...
    db.query(
      query,
      [connectType, connectReply, InvestorID, userID],
      (error, results) => {
        if (error) {
          console.error("Error executing query:", error);
          return res.status(500).json("Error adding to database");
...
app.post(
  "/api/StartupAcceptInvestorConnection",
  authenticateJWT,
  (req, res) => {
...
app.post(
  "/api/AdminDeleteServiceProvider",
  authenticateJWT,
  async (req, res) => {
...
app.get("/api/redirect", (req, res) => {
  ...
  res.redirect(
    `http://localhost:5173/NewStartupForm?userId=${userId}&token=${token}`
  );
});
